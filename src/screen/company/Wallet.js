...
import { getCompanyOrders, requestWithdrawal } from '../../../redux/wallet/walletAction';

const CompanyWallet = () => {
  const dispatch = useDispatch();
  const { user } = useSelector(state => state.auth);
  const walletState = useSelector(state => state.wallet);
  const { orders = [], totalRevenue = 0, isLoading = false } = walletState || {};
  ...
  useEffect(() => {
    if (user) {
      console.log('Company Wallet - User Role:', user.role);
      console.log('Company Wallet - User ID:', user.id);
      dispatch(getCompanyOrders({ page: 1 }));
    }
  }, [user, dispatch]);
  ...
  const handleWithdraw = () => {
    const currentBalance = totalRevenue || 0;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <Header item={"Company Wallet"} showback={true}/>
      </View>
      ...
      <ScrollView
        ...
        <View style={styles.totalAmountSection}>
          <View style={styles.amountHeader}>
            <Text style={styles.totalAmountLabel}>Total Revenue Earned</Text>
            <TouchableOpacity 
          ...
          <Text style={styles.totalAmountValue}>${totalRevenue || 0}</Text>
        </View>
          ...
          {isLoading ? (
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubText}>
                Create products and campaigns to start receiving orders!
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.availableBalanceText}>
              Available Balance ${totalRevenue || 0}
            </Text>
